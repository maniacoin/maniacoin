...
*Setup instructions for a Gitian build of Maniacoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Maniacoin
...
- [Building Maniacoin Core](#building-maniacoin-core)
...
Clone the git repositories for maniacoin and Gitian.
...
in the maniacoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Maniacoin Core
----------------

To build Maniacoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the maniacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/maniacoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/maniacoin-project/maniacoin
...
For example:
```bash
URL=https://github.com/thrasher-/maniacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit maniacoin=${COMMIT} --url maniacoin=${URL} ../maniacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit maniacoin=${COMMIT} --url maniacoin=${URL} ../maniacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit maniacoin=${COMMIT} --url maniacoin=${URL} ../maniacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the maniacoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../maniacoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/maniacoin
SIGPATH=/some/root/path/maniacoin-detached-sigs

./bin/gbuild --url maniacoin=${BTCPATH},signature=${SIGPATH} ../maniacoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/maniacoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/maniacoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/maniacoin-osx-build.assert
```
...
[maniacoin-project/gitian.sigs.ltc](https://github.com/maniacoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
