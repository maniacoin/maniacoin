...
const char * const BITCOIN_CONF_FILENAME = "maniacoin.conf";
const char * const BITCOIN_PID_FILENAME = "maniacoin.pid";
...
#else
    const char* pszModule = "maniacoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Maniacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Maniacoin";
#else
    // Unix
    return pathRet / ".maniacoin";
#endif
#endif
}
